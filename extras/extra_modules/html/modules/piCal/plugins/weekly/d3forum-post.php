<?php
	...
	*/
	...
	$sql = "SELECT p.subject,p.post_id,p.post_time,t.topic_external_link_id,f.forum_id,f.forum_external_link_format FROM ".$db->prefix($plugin['dirname']."_posts")." p LEFT JOIN ".$db->prefix($plugin['dirname']."_topics")." t ON p.topic_id=t.topic_id LEFT JOIN ".$db->prefix($plugin['dirname']."_forums")." f ON f.forum_id=t.forum_id WHERE ! p.invisible AND ! t.topic_invisible AND ($whr_forum) AND ($whr_cat) AND p.post_time >= $range_start_s AND p.post_time < $range_end_s" ;
	$result = $db->query( $sql ) ;
