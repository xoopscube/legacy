<?php
	...
	*/
	...
	$sql = "SELECT t.topic_title,t.topic_id,t.topic_last_post_time,t.topic_external_link_id,f.forum_id,f.forum_external_link_format,p.subject,p.invisible FROM ".$db->prefix($plugin['dirname']."_topics")." t LEFT JOIN ".$db->prefix($plugin['dirname']."_forums")." f ON f.forum_id=t.forum_id LEFT JOIN ".$db->prefix($plugin['dirname']."_posts")." p ON t.topic_last_post_id=p.post_id WHERE ! t.topic_invisible AND ($whr_forum) AND ($whr_cat) AND t.topic_last_post_time >= $range_start_s AND t.topic_last_post_time < $range_end_s" ;
	
	$result = $db->query( $sql ) ;

	while( list( $title , $id , $server_time , $topic_external_link_id , $forum_id , $forum_external_link_format , $post_title , $post_invisible ) = $db->fetchRow( $result ) ) {
