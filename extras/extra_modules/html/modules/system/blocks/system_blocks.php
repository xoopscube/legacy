<?php
// $Id: system_blocks.php,v 1.1 2007/05/15 02:35:34 minahito Exp $
//  ------------------------------------------------------------------------ //
...
function b_system_user_show()
{
    ...
    if (is_object($xoopsUser)) {
		...
		$service=&$root->mServiceManager->searchXCubeService("PrivateMessage");
		if($service) {
			$client=new XCube_ServiceClient($service);
			$parameters=array("uid"=>$xoopsUser->getVar('uid'));
			$block['new_messages'] = $client->call("getCountUnread",$parameters);
