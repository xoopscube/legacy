<?php
// Translation Info
// $Id$
// License http://creativecommons.org/licenses/by/2.5/br/
// ############################################################### //
// ## XOOPS Cube Legacy 2.2 - Tradução para o Português do Brasil
// ############################################################### //
// ## Por............: Mikhail Miguel
// ## E-mail.........: mikhail@underpop.com
// ## Website........: http://xoopscube.com.br
// ## Plus...........: http://card.ly/mikhail
// ############################################################### //
// *************************************************************** //


define("_AD_FILEMANAGER_ACTION_CAPTURE", "Capturar imagem");
define("_AD_FILEMANAGER_ACTION_CONVERT", "Converter para FLV");
define("_AD_FILEMANAGER_ACTION_DEFULT", "&nbsp;Continuar&nbsp;");
define("_AD_FILEMANAGER_ACTION_DELETE", "&nbsp;Remover&nbsp;");
define("_AD_FILEMANAGER_ACTION_SUBMIT", "&nbsp;Enviar&nbsp;");
define("_AD_FILEMANAGER_ADD", "Criar diretório");
define("_AD_FILEMANAGER_ADDFOLDER", "Criar diretório");
define("_AD_FILEMANAGER_ADDFOLDER_CONFIRMMSSAGE", "Criar uma pasta sob %s. <br />Escreva o nome para a nova pasta.");
define("_AD_FILEMANAGER_ADDFOLDER_DSC", "Criar um subdiretório sob o diretório <q>uploads</q> do XOOPS Cube.");
define("_AD_FILEMANAGER_ADDFOLDER_ERROR", "Sem permissão para criar um diretório");
define("_AD_FILEMANAGER_ADDFOLDER_SUCCESS", "Pasta criada corretamente");
define("_AD_FILEMANAGER_CHECK", "Verificar configuração"); // Check System Requirements
define("_AD_FILEMANAGER_CHECK_DSC_1", "Utilização do SWFUpload");
define("_AD_FILEMANAGER_CHECK_DSC_2", "Por motivos de segurança, é recomendável liberar o acesso ao SWFUpload apenas para grupos de confiança, e nunca para uso público. <br /> Crie um arquivo .htaccess para restringir o acesso a esses arquivos.");
define("_AD_FILEMANAGER_CHECK_NG", "<b>Alguns arquivos necessários não foram encontrados:</b><br />");
define("_AD_FILEMANAGER_CHECK_OK", "Verificação concluída, nenhum problema encontrado.");
define("_AD_FILEMANAGER_CONFIRMMSSAGE", "Confirme o envio de %s:");
define("_AD_FILEMANAGER_DATE", "Última alteração");
define("_AD_FILEMANAGER_DEL", "Remover");
define("_AD_FILEMANAGER_DELET", "Remover");
define("_AD_FILEMANAGER_DELFOLDER", "Remover diretório");
define("_AD_FILEMANAGER_DELFOLDER_CONFIRMMSSAGE", "Remover o diretório %s");
define("_AD_FILEMANAGER_DELFOLDER_DSC", "Remover o diretório selecionado.");
define("_AD_FILEMANAGER_DELFOLDER_ERROR", "Não foi possível remover a pasta, que pode não estar vazia ou sem as permissões adequadas.");
define("_AD_FILEMANAGER_DELFOLDER_FILE_EXISTS", "Não foi possível remover a pasta pois o arquivo %s está dentro dela.");
define("_AD_FILEMANAGER_DELFOLDER_ISDIR", "O diretório %s não pode ser removido do gerenciador de arquivos. Por gentileza, utilize um programa FTP para alterar isso.");
define("_AD_FILEMANAGER_DELFOLDER_NOTACCESS", "O diretório %s não pode ser removido pelo gerenciador de arquivos. Por favor, altere as permissões manualmente via software FTP.");
define("_AD_FILEMANAGER_DELFOLDER_SUCCESS", "Pasta removida corretamente.");
define("_AD_FILEMANAGER_EDIT", "Acrescentar");
define("_AD_FILEMANAGER_ERROR_DELETE_FOR_PERMISSION", "Você ainda não tem acesso do tipo <q>%s</q> para o gerenciador de arquivos.");
define("_AD_FILEMANAGER_ERROR_FILE_EXISTS", "%s não encontrado.");
define("_AD_FILEMANAGER_ERROR_FILE_PERMISSION", "%s não tem permissão.");
define("_AD_FILEMANAGER_ERROR_FOLDERNAME", "O nome da pasta não está correto. <br /> Apenas caracteres alfanuméricos, hifem e travessão podem ser utilizados.");
define("_AD_FILEMANAGER_ERROR_PATH", "O nome do caminho para a pasta não está correto. Por favor, verifique se foi escrito corretamente.");
define("_AD_FILEMANAGER_ERROR_PERMISSION", "Não há permissão de acesso para esta operação.");
define("_AD_FILEMANAGER_ERROR_REQUIRED", "{0} é um valor necessário");
define("_AD_FILEMANAGER_FILECOUNT", "Nº de arquivos");
define("_AD_FILEMANAGER_FILENAME", "Nome");
define("_AD_FILEMANAGER_FILE_TOTAL", "Total");
define("_AD_FILEMANAGER_FOLDER", "Pasta");
define("_AD_FILEMANAGER_FOLDERNAME", "Nome da pasta");
define("_AD_FILEMANAGER_FOLDER_ADD", "Criar subdiretório");
define("_AD_FILEMANAGER_FOLDER_UPLOAD", "Enviados para esta pasta");
define("_AD_FILEMANAGER_HTACCESS_DSC_1", "Para gravar as suas alterações, basta clicar no formulário.");
define("_AD_FILEMANAGER_HTACCESS_DSC_2", "Que permitem aceder ao seu endereço IP atual. A composição da amostra é gerado automático. Favor mude para se adequar ao ambiente do seu servidor.");
define("_AD_FILEMANAGER_HTACCESS_PATH", "Arquivo .htaccess para bloquear o acesso indevido");
define("_AD_FILEMANAGER_MAIN_DSC", "Lista dos carregamentos. Quando você clica em uma pasta para mover para a pasta. A grande foto e vídeo preview está disponível");
define("_AD_FILEMANAGER_NOTFOUND", "Arquivo não encontrado.");
define("_AD_FILEMANAGER_NOTFOUNDURL", "O local onde o arquivo seria gravado não foi encontrado, e possivelmente não existe.");
define("_AD_FILEMANAGER_OPTION", "Opção");
define("_AD_FILEMANAGER_OPTION_DSC", "Opção");
define("_AD_FILEMANAGER_PARENT", "Retornar");
define("_AD_FILEMANAGER_PATH_HOME", "Inicial");
define("_AD_FILEMANAGER_PREVIEW", "Mostrar");
define("_AD_FILEMANAGER_RETURN", "Retornar");
define("_AD_FILEMANAGER_SINGLEUPLOAD", "Arquivo");
define("_AD_FILEMANAGER_SIZE", "Tamanho");
define("_AD_FILEMANAGER_SWF_COULD_NOT_LOAD", "Lamento, mas não foi possível carregar o SWFUpload. Você precisa habilitar o JavaScript para desfrutar do SWFUpload.");
define("_AD_FILEMANAGER_SWF_INSTALL_FLASH", "Lamento, mas não foi possível carregar o SWFUpload. Pode ser necessário instalar ou atualizar o Flash Player. <br /> Visite o portal da <a href='http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash' rel='external nofollow' target='_tab'>Adobe</a> para obter a última versão do Flash Player.");
define("_AD_FILEMANAGER_SWF_LOADING", "Carregando a biblioteca SWFUpload. Aguarde ...");
define("_AD_FILEMANAGER_SWF_LOAD_HAS_FAILED", "A biblioteca SWFUpload está levando muito tempo para ser carregado. Por favor, certifique-se de que uma versão recente do Plugin Adobe Flash Player está instalada e funcionando corretamente.");
define("_AD_FILEMANAGER_SWF_UPLOAD_CANCEL", "Cancelar todos os envios de arquivos agendados");
define("_AD_FILEMANAGER_SWF_UPLOAD_CNACEL", "Cancelar todos os envios de arquivos agendados");
define("_AD_FILEMANAGER_SWF_UPLOAD_QUEUE", "Envios agendados");
define("_AD_FILEMANAGER_TYPE", "Tipo");
define("_AD_FILEMANAGER_UPLOAD", "Enviar");
define("_AD_FILEMANAGER_UPLOAD_DSC", "Clique para selecionar para carregar os arquivos do seu computador e iniciar o envio");
define("_AD_FILEMANAGER_UPLOAD_NOFILE", "O arquivo enviado não foi encontrado. Por favor, selecione o arquivo.");
define("_AD_FILEMANAGER_UPLOAD_NOTACCESS", "Não foi possível enviar %s. Por favor, altere as permissões manualmente (por exemplo, com um software FTP).");
define("_AD_FILEMANAGER_UPLOAD_PERMISSION", "O diretório onde os arquivos enviados serão gravados não foi encontrado ou não tem permissão de escrita.");

//
// define("_AD_FILEMANAGER_ACTION_DEFULT", "-----");

?>