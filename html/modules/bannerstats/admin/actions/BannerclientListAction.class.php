...
// It's good practice to include the object definition if you're referencing its class
require_once XOOPS_MODULE_PATH . '/bannerstats/class/BannerClient.class.php'; // Used for mObjects and currentClient
...
class Bannerstats_BannerclientListAction extends Bannerstats_AbstractListAction
{
     ...
     * @return Bannerstats_BannerclientHandler|false handler object
     */
    ...
    /**
     * Gets the filter form for the banner client list
     * @return Bannerstats_BannerclientFilterForm
     */
    public function &_getFilterForm()
    {
        $pageNavi = $this->_getPageNavi();
        $handler = $this->_getHandler();
        $filter = new Bannerstats_BannerclientFilterForm($pageNavi, $handler);
        return $filter;
    }
    ...
    public function executeViewIndex(&$controller, &$xoopsUser, &$render): void
    {
        ...
        if (is_array($this->mObjects)) {
            foreach (array_keys($this->mObjects) as $key) {
                if (isset($this->mObjects[$key]) && $this->mObjects[$key] instanceof Bannerstats_BannerclientObject) {
