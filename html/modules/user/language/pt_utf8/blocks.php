<?php
// ## XOOPS Cube Legacy - Versão em Português
// ## Por............: Mikhail Miguel
// ## E-mail.........: mikhail.miguel@gmail.com
// License https://creativecommons.org/licenses/by/2.5/br/

define( '_MB_USER_USERNAME' , "Codinome:");
define( '_MB_USER_PASSWORD' , "Senha:");
define( '_MB_USER_LOGIN' , "Entrar");
define( '_MB_USER_LOSTPASS' , "Perdeu a senha?");
define( '_MB_USER_USERREG' , "Cadastre-se!");
define( '_MB_USER_SECURE' , "Conexão segura");
define( '_MB_USER_REMEMBERME' , "Lembrar");

define( '_MB_USER_ONLINEPHRASE' , '<span class="badge">%s</span> visitantes');
define( '_MB_USER_ONLINEPHRASEX' , '<span class="badge">%s</span> utilisateur(s) na seção %s');
define( '_MB_USER_MEMBERS' , "Associados");
define( '_MB_USER_GUESTS' , "Anônimos");
define( '_MB_USER_MORE' , "outros...");

define( '_MB_USER_DISPLAY' , "Mostrar %s associados");
define( '_MB_USER_DISPLAYA' , "Mostrar os avatares dos associados");
define( '_MB_USER_NODISPGR' , "Não mostrar os associados de graduação:");

define( '_MB_USER_OAUTH2_DISPLAY_MODE' , 'Display mode'); 
define( '_MB_USER_OAUTH2_DISPLAY_BUTTONS' , 'Providers Buttons');
define( '_MB_USER_OAUTH2_DISPLAY_LINK' , 'Oauth2 Page Link');
define( '_MB_USER_LOGIN_SAML' , 'Login using SAML'); 
define( '_MB_USER_LOGIN_OAUTH2' , 'Login using OAUTH2');
