...
class Profile_DataFilterForm extends Profile_AbstractFilterForm
{
    ...
    **/
    protected function _addSortKeys()
    {
        foreach($this->mFields as $field){
            $this->mSortKeys[$field->getShow('definitions_id')] = $field->get('field_name');
        }
    }
    ...
    **/
    ...
    {
        $this->mFields = xoops_getmodulehandler('definitions', 'profile')->getFields();
    ...
    function fetch()
    {
        ...
        foreach($this->mFields as $field){
            if (($value = $root->mContext->mRequest->getRequest($field->get('field_name'))) !== null) {
                $this->mNavi->addExtra($field->get('field_name', $value));
                $this->_mCriteria->add(new Criteria($field->get('field_name'), $value));
            }
        }
