...
class Blowfish extends Base
{
     ...
     * @access private
     */
    public $parray = [0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0, 0x082efa98, 0xec4e6c89, 0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c, 0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917, 0x9216d5d9, 0x8979fb1b];
    ...
    function _setupKey()
    {
        ...
        $this->bctx = ['p'  => [], 'sb' => [$this->sbox0, $this->sbox1, $this->sbox2, $this->sbox3]];
    ...
    function _bcrypt_hash($sha2pass, $sha2salt)
    {
        ...
        for ($i = 0; $i < 64; $i++) {
            for ($j = 0; $j < 8; $j+= 2) { // count($cdata) == 8
                [$cdata[$j], $cdata[$j + 1]] = $this->_encryptBlockHelperFast($cdata[$j], $cdata[$j + 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
            }
        }
    ...
    function _expand0state($key, &$sbox0, &$sbox1, &$sbox2, &$sbox3, &$p)
    {
        ...
        [$p[0], $p[1]] = $this->_encryptBlockHelperFast(     0,      0, $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[2], $p[3]] = $this->_encryptBlockHelperFast($p[ 0], $p[ 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[4], $p[5]] = $this->_encryptBlockHelperFast($p[ 2], $p[ 3], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[6], $p[7]] = $this->_encryptBlockHelperFast($p[ 4], $p[ 5], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[8], $p[9]] = $this->_encryptBlockHelperFast($p[ 6], $p[ 7], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[10], $p[11]] = $this->_encryptBlockHelperFast($p[ 8], $p[ 9], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[12], $p[13]] = $this->_encryptBlockHelperFast($p[10], $p[11], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[14], $p[15]] = $this->_encryptBlockHelperFast($p[12], $p[13], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[16], $p[17]] = $this->_encryptBlockHelperFast($p[14], $p[15], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        // @codingStandardsIgnoreEnd

        [$sbox0[0], $sbox0[1]] = $this->_encryptBlockHelperFast($p[16], $p[17], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2; $i < 256; $i+= 2) {
            [$sbox0[$i], $sbox0[$i + 1]] = $this->_encryptBlockHelperFast($sbox0[$i - 2], $sbox0[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }

        [$sbox1[0], $sbox1[1]] = $this->_encryptBlockHelperFast($sbox0[254], $sbox0[255], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2; $i < 256; $i+= 2) {
            [$sbox1[$i], $sbox1[$i + 1]] = $this->_encryptBlockHelperFast($sbox1[$i - 2], $sbox1[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }

        [$sbox2[0], $sbox2[1]] = $this->_encryptBlockHelperFast($sbox1[254], $sbox1[255], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2; $i < 256; $i+= 2) {
            [$sbox2[$i], $sbox2[$i + 1]] = $this->_encryptBlockHelperFast($sbox2[$i - 2], $sbox2[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }

        [$sbox3[0], $sbox3[1]] = $this->_encryptBlockHelperFast($sbox2[254], $sbox2[255], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2; $i < 256; $i+= 2) {
            [$sbox3[$i], $sbox3[$i + 1]] = $this->_encryptBlockHelperFast($sbox3[$i - 2], $sbox3[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }
    }
    ...
    function _expandstate($data, $key, &$sbox0, &$sbox1, &$sbox2, &$sbox3, &$p)
    {
        ...
        // @codingStandardsIgnoreStart
        [$p[0], $p[1]] = $this->_encryptBlockHelperFast($data[ 0]         , $data[ 1]         , $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[2], $p[3]] = $this->_encryptBlockHelperFast($data[ 2] ^ $p[ 0], $data[ 3] ^ $p[ 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[4], $p[5]] = $this->_encryptBlockHelperFast($data[ 4] ^ $p[ 2], $data[ 5] ^ $p[ 3], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[6], $p[7]] = $this->_encryptBlockHelperFast($data[ 6] ^ $p[ 4], $data[ 7] ^ $p[ 5], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[8], $p[9]] = $this->_encryptBlockHelperFast($data[ 8] ^ $p[ 6], $data[ 9] ^ $p[ 7], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[10], $p[11]] = $this->_encryptBlockHelperFast($data[10] ^ $p[ 8], $data[11] ^ $p[ 9], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[12], $p[13]] = $this->_encryptBlockHelperFast($data[12] ^ $p[10], $data[13] ^ $p[11], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[14], $p[15]] = $this->_encryptBlockHelperFast($data[14] ^ $p[12], $data[15] ^ $p[13], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        [$p[16], $p[17]] = $this->_encryptBlockHelperFast($data[ 0] ^ $p[14], $data[ 1] ^ $p[15], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        // @codingStandardsIgnoreEnd

        [$sbox0[0], $sbox0[1]] = $this->_encryptBlockHelperFast($data[2] ^ $p[16], $data[3] ^ $p[17], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2, $j = 4; $i < 256; $i+= 2, $j = ($j + 2) % 16) { // instead of 16 maybe count($data) would be better?
            [$sbox0[$i], $sbox0[$i + 1]] = $this->_encryptBlockHelperFast($data[$j] ^ $sbox0[$i - 2], $data[$j + 1] ^ $sbox0[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }

        [$sbox1[0], $sbox1[1]] = $this->_encryptBlockHelperFast($data[2] ^ $sbox0[254], $data[3] ^ $sbox0[255], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2, $j = 4; $i < 256; $i+= 2, $j = ($j + 2) % 16) {
            [$sbox1[$i], $sbox1[$i + 1]] = $this->_encryptBlockHelperFast($data[$j] ^ $sbox1[$i - 2], $data[$j + 1] ^ $sbox1[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }

        [$sbox2[0], $sbox2[1]] = $this->_encryptBlockHelperFast($data[2] ^ $sbox1[254], $data[3] ^ $sbox1[255], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2, $j = 4; $i < 256; $i+= 2, $j = ($j + 2) % 16) {
            [$sbox2[$i], $sbox2[$i + 1]] = $this->_encryptBlockHelperFast($data[$j] ^ $sbox2[$i - 2], $data[$j + 1] ^ $sbox2[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }

        [$sbox3[0], $sbox3[1]] = $this->_encryptBlockHelperFast($data[2] ^ $sbox2[254], $data[3] ^ $sbox2[255], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        for ($i = 2, $j = 4; $i < 256; $i+= 2, $j = ($j + 2) % 16) {
            [$sbox3[$i], $sbox3[$i + 1]] = $this->_encryptBlockHelperFast($data[$j] ^ $sbox3[$i - 2], $data[$j + 1] ^ $sbox3[$i - 1], $sbox0, $sbox1, $sbox2, $sbox3, $p);
        }
    }
    ...
    function _setupInlineCrypt()
    {
        ...
        if (!isset($lambda_functions[$code_hash])) {
            ...
            $lambda_functions[$code_hash] = $this->_createInlineCryptFunction(
                ['init_crypt'    => $init_crypt, 'init_encrypt'  => '', 'init_decrypt'  => '', 'encrypt_block' => $encrypt_block, 'decrypt_block' => $decrypt_block]
            );
        }
