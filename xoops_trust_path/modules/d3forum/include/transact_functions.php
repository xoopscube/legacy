...
function d3forum_main_d3comment_callback( $mydirname, $topic_id, $mode = 'update', $post_id = 0 ) {
	...
	[
		...
		$external_link_id,
		$forum_id
	] = $db->fetchRow( $db->query( 'SELECT f.forum_external_link_format,t.topic_external_link_id,t.forum_id FROM '
	                               . $db->prefix( $mydirname . '_topics' ) . ' t LEFT JOIN '
	                               . $db->prefix( $mydirname . '_forums' ) . " f ON f.forum_id=t.forum_id WHERE topic_id=$topic_id" ) );
...
function d3forum_sync_category( $mydirname, $cat_id ) {
	...
	[
		$last_post_id_in_tree,
		$last_post_time_in_tree,
		$topics_count_in_tree,
		$posts_count_in_tree
	] = $db->fetchRow( $result );

	// update query
	if ( ! $result = $db->queryF( 'UPDATE '
	                              . $db->prefix( $mydirname . '_categories' ) . ' SET cat_topics_count='
	                              . (int) $topics_count . ',cat_posts_count='
	                              . (int) $posts_count . ', cat_last_post_id='
	                              . (int) $last_post_id . ', cat_last_post_time='
	                              . (int) $last_post_time . ',cat_topics_count_in_tree='
...
function d3forum_sync_topic( $mydirname, $topic_id, $sync_also_forum = true, $sync_topic_title = false ) {
	...
	if ( empty( $total_posts ) ) {
		...
		[
			...
			$first_subject,
			$unique_path
		] = $db->fetchRow( $db->query( 'SELECT post_time,uid,subject,unique_path FROM ' . $db->prefix( $mydirname . '_posts' ) . " WHERE post_id=$first_post_id" ) );

		[
			$last_post_time,
			$last_uid
		] = $db->fetchRow( $db->query( 'SELECT post_time,uid FROM ' . $db->prefix( $mydirname . '_posts' ) . " WHERE post_id=$last_post_id" ) );
...
function d3forum_maketree_recursive( $tablename, $post_id, $order = 'post_id', $parray = [], $depth = 0, $unique_path = '.1' ) {
	...
	while ( [$new_post_id, $new_unique_path] = $db->fetchRow( $result ) ) {
		...
		if ( ! empty( $new_unique_path ) ) {
			...
			$count_of_last_level = (int) substr( strrchr( $new_unique_path, '.' ), 1 );
...
function d3forum_cutpasteposts( $mydirname, $post_id, $pid, $new_forum_id, $forum_permissions, $isadmin ) {
	...
	if ( 0 === $pid ) {
		...
		[
			$new_forum_id,
			$new_forum_external_link_format
		] = $db->fetchRow( $db->query( 'SELECT forum_id,forum_external_link_format FROM ' . $db->prefix( $mydirname . '_forums' ) . " WHERE forum_id=$new_forum_id" ) );
		...
		[
			...
			$topic_id,
			$subject
		] = $db->fetchRow( $db->query( 'SELECT pid,topic_id,subject FROM ' . $db->prefix( $mydirname . '_posts' ) . " WHERE post_id=$post_id" ) );

		if ( $pid ) {
			// get external_link_id of the current topic
			[ $external_link_id ] = $db->fetchRow( $db->query( 'SELECT topic_external_link_id FROM ' . $db->prefix( $mydirname . '_topics' ) . " WHERE topic_id=$topic_id" ) );
			// create a new topic and copy subject to topic_title in sync
			if ( ! $db->query( 'INSERT INTO ' . $db->prefix( $mydirname . '_topics' ) . " SET forum_id=$new_forum_id, topic_title='" . addslashes( $subject ) . "', topic_external_link_id='" . addslashes( $external_link_id ) . "'" ) ) {
			...
			if ( ! $db->query( 'UPDATE ' . $db->prefix( $mydirname . '_posts' ) . " SET pid=0 WHERE post_id=$post_id" ) ) {
			...
			if ( ! $db->query( 'UPDATE ' . $db->prefix( $mydirname . '_topics' ) . " SET forum_id=$new_forum_id WHERE topic_id=$topic_id" ) ) {
		...
		if ( ( '' === $new_forum_external_link_format ) && ! $db->query( 'UPDATE ' . $db->prefix( $mydirname . '_topics' ) . " SET topic_external_link_id='' WHERE topic_id=$new_topic_id" ) ) {
		...
		[
			...
			$new_topic_id,
			$new_forum_id
		] = $db->fetchRow( $db->query( 'SELECT p.post_id,t.topic_id,t.forum_id FROM ' . $db->prefix( $mydirname . '_posts' ) . ' p LEFT JOIN ' . $db->prefix( $mydirname . '_topics' ) . ' t ON p.topic_id=t.topic_id LEFT JOIN ' . $db->prefix( $mydirname . '_forums'
...
function d3forum_update_topic_from_post( $mydirname, $topic_id, $forum_id, $forum_permissions, $isadmin ) {
	...
	$topic_locked     = (int) @$_POST['topic_locked'];
	$topic_invisible  = (int) @$_POST['topic_invisible'];
	$topic_solved     = (int) @$_POST['topic_solved'];
			...
			if ( preg_match( '/^' . $key . '\:(.{1,100})$/', $line, $regs ) ) {
				switch ( $d3forum_configs_can_be_override[ $key ] ) {
					case 'text' :
						...
						break;
					case 'int' :
						$options[ $key ] = (int) $regs[1];
						break;
					case 'bool' :
						$options[ $key ] = (int) $regs[1] > 0 ? 1 : 0;
						break;
				}
			}
		}
