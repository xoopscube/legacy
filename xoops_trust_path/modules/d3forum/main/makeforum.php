...
if ( ! $isadmin && ! @$category_permissions[ $cat_id ]['can_makeforum'] && ! @$category_permissions[ $cat_id ]['is_moderator'] ) {
	die( _MD_D3FORUM_ERR_CREATEFORUM );
}
// TRANSACTION PART
...
if ( isset( $_POST['forumman_post'] ) ) {
	...
	[ $forum_id, $forum_title ] = d3forum_makeforum( $mydirname, $cat_id, $isadmin );
...
$xoopsTpl->assign( [
		...
		'xoops_breadcrumbs'   => array_merge( $xoops_breadcrumbs, [ [ 'name' => _MD_D3FORUM_LINK_MAKEFORUM ] ] ),
	]
);
