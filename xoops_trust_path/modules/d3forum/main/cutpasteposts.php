...
if ( ! empty( $_POST['cutpastepostsok'] ) ) {
	...
	if ( empty( $new_pid ) && empty( $new_forum_id ) ) {
		die( _MD_D3FORUM_ERR_NOSPECIFICID );
	}

	[
		$new_topic_id,
		$new_forum_id
	] = d3forum_cutpasteposts( $mydirname, $post_id, $new_pid, $new_forum_id, $forum_permissions, $isadmin );
