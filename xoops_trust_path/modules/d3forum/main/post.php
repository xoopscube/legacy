...
if ( 'edit' == $mode ) {
	...
	else if ( ! $isadminormod && ( $post_row['uid'] != $xoopsUser->getVar( 'uid' ) || time() >= $post_row['post_time'] + $xoopsModuleConfig['selfeditlimit'] ) ) {
		die( _MD_D3FORUM_ERR_EDITPOST );
	}
} else if ( 'reply' == $mode ) {
...
if ( empty( $xoopsModuleConfig['allow_sig'] ) ) {
	$allow_sig = 0;
}
...
if ( $html ) {
	$message = d3forum_transact_htmlpurify( $message, $mydirname );
}
...
if ( ! empty( $_POST['contents_preview'] ) ) {
	...
	if ( 'edit' != $mode && 0 == $uid || 'edit' == $mode && 0 == $post_row['uid'] && 0 == $post_row['uid_hidden'] ) {
		...
		[ $guest_name, $trip_base ] = explode( '#', $guest_name, 2 );

		if ( ! trim( @$guest_name ) ) {
			$guest_name = $xoopsModuleConfig['anonymous_name'];
		}
	...
	if ( 'edit' == $mode ) {
		...
		if ( ! $db->query( 'INSERT INTO ' . $db->prefix( $mydirname . '_posts' ) . " SET $set4sql,pid=$pid,topic_id=$topic_id,post_time=UNIX_TIMESTAMP(),poster_ip='" . addslashes( @$_SERVER['REMOTE_ADDR'] ) . "'" ) ) {
