...
define('HWLDF_NBSP', "\xC2\xA0");     // utf-8 non-breaking space. // by nao-pon
//define('NBSP', "\xC2\xA0");     // utf-8 non-breaking space.

class _HWLDF_WordAccumulator {
  var $nbsp; // by nao-pon
//  function _HWLDF_WordAccumulator ()) {
  function _HWLDF_WordAccumulator ($options = array()) { // by nao-pon
    $this->_lines = array();
    $this->_line = '';
    $this->_group = '';
    $this->_tag = '';
    ...
    if (isset($options['nbsp'])) {
    	$this->nbsp = $options['nbsp'];
    } else {
    	$this->nbsp = HWLDF_NBSP;
    }
  }
  ...
  function addWords ($words, $tag = '') {
    ...
    foreach ($words as $word) {
      ...
      if ($word == '')
        continue;
      if ($word[0] == "\n") {
        $this->_group .= $this->nbsp; // by nao-pon
        //$this->_group .= NBSP;
        $this->_flushLine($tag);
        $word = substr($word, 1);
      }
...
{
  var $_WordAccumulator_options; // by nao-pon

  function WordLevelDiff ($orig_lines, $closing_lines) {
    $this->_WordAccumulator_options['nbsp'] = '&nbsp;'; // by nao-pon
...
#    if (!preg_match_all('/ ( [^\S\n]+ | [[:alnum:]]+ | . ) (?: (?!< \n) [^\S\n])? /xs',
  ...
  function orig () {
    $orig = new _HWLDF_WordAccumulator($this->_WordAccumulator_options); // by nao-pon
  ...
  function closing () {
    $closing = new _HWLDF_WordAccumulator($this->_WordAccumulator_options); // by nao-pon
...
class TableDiffFormatter extends DiffFormatter
{
  ...
  function _block_header( $xbeg, $xlen, $ybeg, $ylen ) {
    ...
    $l1 = $lang['line'].' '.$xbeg;
    $l2 = $lang['line'].' '.$ybeg;
    $r = '<tr><td class="diff-blockheader" colspan="2">Line '.$l1.":</td>\n" .
      '<td class="diff-blockheader" colspan="2">Line '.$l2.":</td></tr>\n";
    return $r;
  }
