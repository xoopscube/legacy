<?php
class xpwiki_plugin_md5 extends xpwiki_plugin {
	function plugin_md5_init () {



	}
	// PukiWiki - Yet another WikiWikiWeb clone.
	// $Id: md5.inc.php,v 1.1 2006/10/13 13:17:49 nao-pon Exp $
	...
	//	global $get, $post;
		...
		if ($this->cont['PKWK_SAFE_MODE'] || $this->cont['PKWK_READONLY']) $this->func->die_message('Prohibited by admin');
	
		// Wait POST
		$phrase = isset($this->root->post['phrase']) ? $this->root->post['phrase'] : '';
	
		if ($phrase == '') {
			// Show the form
	
			// If plugin=md5&md5=password, only set it (Don't compute)
			$value  = isset($this->root->get['md5']) ? $this->root->get['md5'] : '';
	
			return array(
				'msg' =>'Compute userPassword',
			'body'=>$this->plugin_md5_show_form(isset($this->root->post['phrase']), $value));
	
		} else {
			// Compute (Don't show its $phrase at the same time)
	
			$prefix = isset($this->root->post['prefix']);
			$salt   = isset($this->root->post['salt']) ? $this->root->post['salt'] : '';
	
			// With scheme-prefix or not
			if (! preg_match('/^\{.+\}.*$/', $salt)) {
				$scheme = isset($this->root->post['scheme']) ? '{' . $this->root->post['scheme'] . '}': '';
				$salt   = $scheme . $salt;
			}
			...
			'body'=>
					//($prefix ? 'userPassword: ' : '') .
					$this->func->pkwk_hash_compute($phrase, $salt, $prefix, TRUE));
		}
	}
	...
	{
		if ($this->cont['PKWK_SAFE_MODE'] || $this->cont['PKWK_READONLY']) $this->func->die_message('Prohibited');
...
EOD;
  ...
  <label for="_p_md5_lssha">LDAP $this->func->SSHA (sha-1 with a seed) *</label><br />
  <input type="radio" name="scheme" id="_p_md5_lsha" value="SHA" />
  <label for="_p_md5_lsha">LDAP $this->func->SHA (sha-1)</label><br />
EOD;
	
		$form .= <<<EOD
  <input type="radio" name="scheme" id="_p_md5_lsmd5" value="SMD5" $md5_checked/>
  <label for="_p_md5_lsmd5">LDAP $this->func->SMD5 (md5 with a seed) *</label><br />
  ...
  <label for="_p_md5_prefix">Add scheme $this->func->prefix (RFC2307, Using LDAP as NIS)</label><br />
