...
eval( ' function xoops_module_install_'.$mydirname.'( $module ) { return d3pipes_oninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'd3pipes_oninstall_base' ) ) {

function d3pipes_oninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleInstall.' . ucfirst($mydirname) . '.Success' , 'd3pipes_message_append_oninstall' ) ;
		$ret = array() ;
	} else {
...
function d3pipes_message_append_oninstall( &$module_obj , &$log )
{
