...
eval( ' function xoops_module_uninstall_'.$mydirname.'( $module ) { return d3pipes_onuninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'd3pipes_onuninstall_base' ) ) {

function d3pipes_onuninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleUninstall.' . ucfirst($mydirname) . '.Success' , 'd3pipes_message_append_onuninstall' ) ;
		$ret = array() ;
	} else {
...
function d3pipes_message_append_onuninstall( &$module_obj , &$log )
{
