...
define($constpref.'_JQUERY_REMOVE_FLASH' , 'Flash除去(jqm)');
define($constpref.'_JQUERY_REMOVE_FLASH_DESC' , 'jQuery Mobile 適用時に Flash を除去するプロファイル名をカンマ区切りで記述。プロファイル名は携帯対応レンダラーで定義されていて、docomo, au, softbank, willcom, android, iphone, ipod, ipad, windows mobile などが使用できます。');
define($constpref.'_JQUERY_RESOLVE_TABLE' , '入れ子テーブル展開(jqm)');
define($constpref.'_JQUERY_RESOLVE_TABLE_DESC' , 'jQuery Mobile 適用時に入れ子になっているテーブルを展開する。');
define($constpref.'_JQUERY_IMAGE_CONVERT' , '最大画像幅[px](jqm)');
define($constpref.'_JQUERY_IMAGE_CONVERT_DESC' , 'jQuery Mobile 適用時に画像を指定幅[px]サイズまで縮小する。「0」で無効になります。');
