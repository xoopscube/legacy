...
eval( ' function xoops_module_uninstall_'.$mydirname.'( $module ) { return hypconf_onuninstall_base( $module , "'.$mydirname.'" ) ; } ' ) ;


if( ! function_exists( 'hypconf_onuninstall_base' ) ) {

function hypconf_onuninstall_base( $module , $mydirname )
{
	...
	if( defined( 'XOOPS_CUBE_LEGACY' ) ) {
		...
		$root->mDelegateManager->add( 'Legacy.Admin.Event.ModuleUninstall.' . ucfirst($mydirname) . '.Success' , 'hypconf_message_append_onuninstall' ) ;
		$ret = array() ;
	} else {
		if( ! is_array( $ret ) ) $ret = array() ;
	}

	$file = '/cache/hypconf_'.urlencode(substr(XOOPS_URL, 7)).'.conf';
	if (unlink(XOOPS_TRUST_PATH . $file)) {
		$ret[] = 'Deleted a file "trust' . $file . '"';
	}
...
function hypconf_message_append_onuninstall( &$module_obj , &$log )
{
