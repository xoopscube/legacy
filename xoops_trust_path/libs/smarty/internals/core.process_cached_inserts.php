...
function smarty_core_process_cached_inserts($params, &$smarty)
{
    ...
    for ($i = 0, $for_max = is_countable($cached_inserts) ? count($cached_inserts) : 0; $i < $for_max; $i++) {
        ...
        if ($smarty->debugging) {
            $_params = [];
            require_once(SMARTY_CORE_DIR . 'core.get_microtime.php');
            $smarty->_smarty_debug_info[] = ['type'      => 'insert', 'filename'  => 'insert_'.$name, 'depth'     => $smarty->_inclusion_depth, 'exec_time' => smarty_core_get_microtime($_params, $smarty) - $debug_start_time];
        }
    }
