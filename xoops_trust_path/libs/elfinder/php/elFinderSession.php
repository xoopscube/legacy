...
class elFinderSession implements elFinderSessionInterface
{
     ...
     * @var        array
     */
    protected $opts = ['base64encode' => false, 'keys' => ['default' => 'elFinderCaches', 'netvolume' => 'elFinderNetVolumes'], 'cookieParams' => []];
    ...
    {
        set_error_handler([$this, 'session_start_error'], E_NOTICE | E_WARNING);
        ...
        if ($this->started) {
            if ($this->fixCookieRegist === true) {
                ...
                if (version_compare(PHP_VERSION, '7.3', '<')) {
                    setcookie(session_name(), session_id(), ['expires' => 0, 'path' => $cParm['path'] . (!empty($cParm['SameSite'])? '; SameSite=' . $cParm['SameSite'] : ''), 'domain' => $cParm['domain'], 'secure' => $cParm['secure'], 'httponly' => $cParm['httponly']]);
                } else {
