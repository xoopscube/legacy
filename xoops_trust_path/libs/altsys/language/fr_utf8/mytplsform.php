<?php

define( '_MD_A_MYTPLSFORM_TPLSADMIN' , 'Templates');
define( '_MD_A_MYTPLSFORM_EDIT' , 'Edition du template');
define( '_MD_A_MYTPLSFORM_UPDATED' , 'Mise à jour avec succés du template');
define( '_MD_A_MYTPLSFORM_CREATED' , 'Template créé avec succés');
define( '_MD_A_MYTPLSFORM_LABEL_TPLFILE' , 'Nom du Template');
define( '_MD_A_MYTPLSFORM_BTN_MODIFYCONT' , 'Modifier');
define( '_MD_A_MYTPLSFORM_BTN_MODIFYEND' , 'Sauvegarder');
define( '_MD_A_MYTPLSFORM_BTN_CREATE' , 'Créer');
define( '_MD_A_MYTPLSFORM_BTN_RESET' , 'Réinitialiser');
define( '_MD_A_MYTPLSFORM_CUSTOM_TIPS' , 'Custom Template Tip.');
